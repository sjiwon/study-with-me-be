...
import static com.kgu.studywithme.favorite.exception.FavoriteErrorCode.ALREADY_LIKE_MARKED;
...
public class FavoriteAcceptanceTest extends AcceptanceTest {
    ...
    class LikeMarkingApi {
        @Test
        @DisplayName("이미 찜 등록한 스터디는 중복 등록할 수 없다")
        void throwExceptionByAlreadyLikeMarked() {
            스터디를_찜_등록한다(accessToken, studyId);

            스터디를_찜_등록한다(accessToken, studyId)
                    .statusCode(ALREADY_LIKE_MARKED.getStatus().value())
                    .body("errorCode", is(ALREADY_LIKE_MARKED.getErrorCode()))
                    .body("message", is(ALREADY_LIKE_MARKED.getMessage()));
        }
