...
class MemberReportTest extends ParallelTest {
    ...
    void construct() {
        // when
        final MemberReport memberReport = MemberReport.createReport(memberA, memberB, reason);

        // then
        assertAll(
                () -> assertThat(memberReport.getReporter()).isEqualTo(memberA),
                () -> assertThat(memberReport.getReportee()).isEqualTo(memberB),
