...
public class ParticipationInspector {
    ...
    public void checkApplierIsHost(final Study study, final Long applierId) {
        if (study.isHost(applierId)) {
            throw StudyWithMeException.type(StudyParticipantErrorCode.STUDY_HOST_CANNOT_APPLY);
        }
    }

    public void checkApplierIsAlreadyRelatedToStudy(final Study study, final Long applierId) {
    ...
    public void checkNewHostIsCurrentHost(final Study study, final Member participant) {
        if (study.isHost(participant.getId())) {
