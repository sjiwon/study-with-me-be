...
CURRENT_RUNNING_PID=$(sudo docker container ls -q)
...
sudo docker run -d --name application -p 8080:8080 -v $DEPLOY_PATH/logs:/app/logs $DOCKER_IMAGE:$DOCKER_IMAGE_TAG
